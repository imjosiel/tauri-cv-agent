...
export default function App() {
  ...
  // Estado de instalação do TinyTeX
  ...
  useEffect(() => {
    ...
    on("texlive_progress", (p: { pct: number; message: string }) => {
      if (p.pct >= 100) {
        // Mantém a tela por 1s mostrando "pronto", depois some
    ...
    on("job_analyzed", (p) => {
      addEvent("analyzed", p);
      saveJob(p, "analyzed", { score: p.score ?? null });
    });
    on("job_applied", (p) => {
      addEvent("applied", p);
      saveJob(p, "applied", {
  ...
  return (
    <div className={styles.shell}>
      ...
      {/* Overlay de instalação do TinyTeX */}
      {texInstall.active && (
        <div className={styles.texOverlay}>
          <div className={styles.texCard}>
            ...
            <div className={styles.texBarWrap}>
              <div
                className={styles.texBar}
                style={{ width: `${texInstall.pct}%` }}
              />
            </div>
